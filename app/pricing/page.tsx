...
function PlanCard({ title, price, per, note, href }: { title: string; price: string; per: string; note: string; href?: string }) {
  return (
    <div className="rounded-xl2 border border-sand bg-white/70 p-7 shadow-sm">
      <h3 className="text-lg font-extrabold">{title}</h3>
      <p className="mt-3 text-4xl font-extrabold tracking-tight">
        {price} <span className="text-base font-bold text-ink-soft">{per}</span>
      </p>
      <p className="mt-2 text-sm text-ink-soft">{note}</p>
      {href ? (
        <a href={href} className="mt-6 inline-block rounded-full bg-amber px-6 py-3 font-extrabold text-white hover:bg-amber-deep">
          Choose {title.toLowerCase()}
        </a>
      ) : (
        <p className="mt-6 rounded-full bg-sand px-5 py-3 text-center text-sm font-bold text-ink-soft">Bought in the app, after the trial</p>
      )}
    </div>
  );
}
    ...
    <PageShell>
        ...
        <ul className="mt-10 grid gap-3 sm:grid-cols-2">
          {included.map((line) => (
            <li key={line} className="flex gap-3 text-ink-soft">
              <span aria-hidden="true" className="font-extrabold text-amber">✓</span>
              {line}
            </li>
          ))}
        </ul>
      ...
      <Section tone="sand">
        <div className="grid gap-10 lg:grid-cols-2">
          <div>
            <h2 className="text-3xl font-extrabold tracking-tight">The cube</h2>
          ...
          <div id="waitlist">
            <h3 className="text-xl font-extrabold">Tell me when it ships</h3>
            <p className="mt-2 mb-5 text-ink-soft">One email, when there is a cube to buy.</p>
            <WaitlistForm />
          </div>
        </div>
      </Section>
      <Section>
        <h2 className="text-3xl font-extrabold tracking-tight">Plan questions</h2>
